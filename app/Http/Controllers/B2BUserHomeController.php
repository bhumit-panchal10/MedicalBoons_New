...
class B2BUserHomeController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $B2BOrders = CorporateOrder::orwhere('main_parent_id', $sessionUsers_id)
            ->orwhere('parent_id', $sessionUsers_id)
            ->count();
        $orderIds = CorporateOrder::orwhere('main_parent_id', $sessionUsers_id)
            ->orwhere('parent_id', $sessionUsers_id)
            ->pluck('Corporate_Order_id');
        //$orderIds = CorporateOrder::where('iUserId', $sessionUsers_id)->pluck('Corporate_Order_id');
        $member = Member::whereIn('Order_id', $orderIds)->count();

        return view('dashboard.home_b2buser', compact('B2BOrders', 'member'));
    }

    public function B2BUsergetProfile()
    {
        $sessionUsers_id = session('Users_id');
        $users = Users::where('Users_id',  $sessionUsers_id)->first();

        return view('B2Buserprofile', compact('users'));
    }
    ...
    {
        return view('dashboard.B2BChangepassword');
    }
