...
@section('title', 'Edit Lab Test Master')
@section('content')
    {!! Toastr::message() !!}
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                ...
                <ul class="flex items-center gap-2 text-sm font-normal shrink-0">
                    ...
                    <li
                        class="relative before:content-['\ea54'] before:font-remix ltr:before:-right-1 rtl:before:-left-1  before:absolute before:text-[18px] before:-top-[3px] ltr:pr-4 rtl:pl-4 before:text-slate-400 dark:text-zink-200">
                        <a href="{{ route('associated_member.index') }}" class="text-slate-400 dark:text-zink-200">Lab Test
                            Master
                            List</a>
                    </li>
                    <li class="text-slate-700 dark:text-zink-100">
                        Edit Lab Test Master
                    </li>
                </ul>
            </div>
                                ...
                                <div class="rtl:md:text-start">
                                    <div class="bg-white shadow rounded-md dark:bg-zink-600">
                                        <div
                                            ...
                                            <h5 class="text-16" id="exampleModalLabel">Edit Lab Test Master</h5>
                                        ...
                                        <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                                            <form onsubmit="return validateFile()" class="tablelist-form"
                                                action="{{ route('lab_test_master.update', $data->id) }}" method="POST">
                                                @csrf

                                                <div class="grid grid-cols-3 gap-4">
                                                    <input type="hidden" name="Lab_Test_Master_id"
                                                        value="{{ $data->Lab_Test_Master_id }}">
                                                    <div class=" mb-3">
                                                        <label for="email-field" class="">Category<span
                                                                class="text-red-500">*</span></label>
                                                        <select name="editlabcatid" id="editlabcatid" required
                                                            class="form-select border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                                                            <option selected="" value="">Select Service</option>
                                                            @foreach ($LabTestCategory as $LabTestCat)
                                                                <option value="{{ $LabTestCat->Lab_Test_Category_id }}"
                                                                    {{ $data->lab_test_category_id == $LabTestCat->Lab_Test_Category_id ? 'selected' : '' }}>
                                                                    {{ $LabTestCat->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <div class="mb-3">
                                                        <label for="email-field" class="">Test Name<span
                                                                class="text-red-500">*</span></label>
                                                        <input type="text" id="email-field" name="edit_test_name"
                                                            maxlength="150"
                                                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                            placeholder="Enter Test Name" required autocomplete="off"
                                                            autofocus value="{{ $data->Test_Name }}">
                                                    </div>

                                                    <div class="mb-3">
                                                        <label for="email-field" class="">MRP<span
                                                                class="text-red-500">*</span></label>
                                                        <input type="text" id="email-field" name="edit_MRP"
                                                            maxlength="150"
                                                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
