    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Corporate User List</h5>
                </div>
                            ...
                            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                                ...
                                <div class="rtl:md:text-start">
                                    ...
                                    <a class="mx-1 ltr:md:text-end" title="Add New Career"
                                        href="{{ route('Corporate_User.add') }}" style="float: inline-end;">
                                        <button type="submit"
                                            class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                            <i class="ri-add-box-fill"></i> Add Corporate User
                                        </button>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="overflow-x-auto">
                                @if (!$CorporateUsers->isEmpty())
                                    <form id="bulkDeleteForm" method="POST"
                                        action="{{ route('Corporate_User.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                                    ...
                                                    <tr>
                                                        <th class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500"
                                                            ...
                                                            <input
                                                                ...
                                                                value="{{ $CorporateUser->Users_id }}">
                                                        </th>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 id">
                                                            {{ $i + $CorporateUsers->perPage() * ($CorporateUsers->currentPage() - 1) }}
                                                        </td>
                                                        ...
                                                        <td
                                                            ...
                                                            {{ $CorporateUser->company_name }}</td>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ $CorporateUser->contact_person }}</td>
                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ $CorporateUser->mobile }}</td>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ $CorporateUser->email }}</td>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500">
                                                            <div class="flex gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="ResentMail"
                                                                        href="{{ route('Corporate_User.Resentmail', $CorporateUser->Users_id) }}">
                                                                    ...
                                                                    <a class="mx-1" title="Edit" href="#"
                                                                        ...
                                                                        onclick="editpassword(<?= $CorporateUser->Users_id ?>)">
                                                                        <i class="fa fa-key" aria-hidden="true"></i>
                                                                    </a>
                                                                    <a class="mx-1" title="Edit"
                                                                        href="{{ route('Corporate_User.edit', $CorporateUser->Users_id) }}">
                                                                        <i class="ri-edit-2-fill"></i>
                                                                    </a>

                                                                    <a class="mx-1" title="Delete" href="#"
                                                                        onclick="confirmSingleDelete({{ $CorporateUser->Users_id }})">
                                    ...
                                    <div class="flex items-center justify-between mt-5">
                                        {{ $CorporateUsers->appends(request()->except('page'))->links() }}
                                    </div>
                                @else
                                    <div class="noresult">
        ...
        <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
            ...
            <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                <form class="tablelist-form" onsubmit="return EditvalidateFile()"
                    action="{{ route('Corporate_User.passwordupdate') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" id="corporateuser_id" name="corporateuser_id" />
                    <div class="mb-3">
    ...
    <script>
        ...
        function confirmSingleDelete(id) {
            Swal.fire({
                ...
                if (result.value) {
                    fetch('{{ route('Corporate_User.delete', ['id' => '__id__']) }}'.replace('__id__', id), {
                            method: 'DELETE',
                            headers: {
                        ...
                        .then(data => {
                            if (data.success) {
                                Swal.fire({
                                    ...
                                    window.location.href = `{{ route('Corporate_User.index') }}`;
                                });
                            } else {
                                Swal.fire({
                                    title: 'Error!',
