...
@section('content')
    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Offer List</h5>
                </div>
                        ...
                        <div class="card-body">
                            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                                ...
                                <div class="rtl:md:text-start">
                                    ...
                                    @endif
                                    <a href="{{ route('offer.create') }}" style="float: inline-end;">
                                        <button type="submit" title="Add New Offer"
                                            class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20"
                                            data-modal-target="AddModal">
                                            <i class="ri-add-box-fill"></i> Add New Offer
                                        </button>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="overflow-x-auto">
                                ...
                                @if (!$offers->isEmpty())
                                    <form id="bulkDeleteForm" method="POST" action="{{ route('offer.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                        <table class="w-full whitespace-nowrap" id="customerTable">
                                            <thead class="bg-slate-100 dark:bg-zink-600">
                                                <tr>
                                                    ...
                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 "
                                                        data-sort="state_name">Code </th>

                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 "
                                                        data-sort="state_name">Value (%) </th>

                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 "
                                                        data-sort="state_name">From Date </th>

                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 "
                                                        data-sort="state_name">To Date </th>
                                                                ...
                                                                <div
                                                                    ...
                                                                    @if ($offer->iStatus == 1)
                                                                        <input type="checkbox" name="greenIconSwitch"
                                                                            ...
                                                                            onchange="updateStatus(<?= $offer->iStatus ?>,<?= $offer->id ?>);"
                                                                            checked="">
                                                                    @else
                                                                        <input type="checkbox" name="greenIconSwitch"
                                                                            id="greenIconSwitch{{ $offer->id }}"
                                                                            ...
                                                                            onchange="updateStatus(<?= $offer->iStatus ?>,<?= $offer->id ?>);">
                                                                    @endif
                                                                    <label for="greenIconSwitch{{ $offer->id }}"
                                                            ...
                                                            <div class="gap-2">
                                                                <div class="edit">
                                                                    ...
                                                                    <a class="mx-1" title="Delete" href="#"
                                                                        onclick="confirmSingleDelete({{ $offer->id }})">
    ...
    <script>
        function updateStatus(status, offerId) {
            //$("p").toggle();
            $('#loading').css("display", "block");
            $.ajax({
                type: 'POST',
                url: "{{ route('offer.updateStatus') }}",
                data: {
                    status: status,
                    offerId: offerId
                },
    ...
    <script>
        function checkAll(source) {
            checkboxes = document.querySelectorAll('input[name="offer_ids[]"]');
            checkboxes.forEach(checkbox => checkbox.checked = source.checked);
        }

        function confirmBulkDelete() {
            const selected = document.querySelectorAll('input[name="offer_ids[]"]:checked');
        ...
        function confirmSingleDelete(id) {
            Swal.fire({
                ...
                if (result.value) {
                    fetch(`{{ route('offer.delete', ['id' => '__id__']) }}`.replace('__id__', id), {
                            method: 'DELETE',
                            headers: {
