...
@section('content')
    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Edit Member Plan</h5>
                </div>
            ...
            <div class="card" id="customerList">
                <div class="card-body">
                    ...
                    <div class="bg-white shadow rounded-md dark:bg-zink-600">
                        ...
                        <div class="p-4">
                            <form class="tablelist-form" action="{{ route('Retail_Customer.update', $member->id) }}"
                                method="POST">
                                @csrf
                                ...
                                <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                                    ...
                                    <div class="xl:col-span-4">
                                        ...
                                        <input type="text" name="state"
                                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                            value="{{ old('state', $member->state) }}" required>
                                    </div>
                                        ...
                                        <select name="plan_id" id="plan_id"
                                            ...
                                            @foreach ($plans as $plan)
                                                <option value="{{ $plan->id }}"
                                                    {{ old('plan_id', $member->iPlanId) == $plan->id ? 'selected' : '' }}>
                                                    {{ $plan->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="xl:col-span-4">
                                        ...
                                        <select name="extra_member" id="extra_member"
                                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                                            <option value="0"
                                                {{ old('extra_member', $member->iExtraMember) == 0 ? 'selected' : '' }}>0
                                            </option>
                                            @for ($i = 1; $i <= 5; $i++)
                                                <option value="{{ $i }}"
                                                    {{ old('extra_member', $member->iExtraMember) == $i ? 'selected' : '' }}>
                                                    {{ $i }}
                                                </option>
                                            @endfor
                                        </select>
                                    ...
                                    <div class="xl:col-span-4">
                                        ...
                                        <input type="date" name="expiry_date" id="expiry_date"
                                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                            value="{{ old('expiry_date', $member->end_date) }}" required>
                                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const selectedPlanId = document.getElementById('plan_id').value;
