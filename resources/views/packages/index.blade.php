...
@section('content')
    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Packages List</h5>
                </div>
                        ...
                        <div class="card-body">
                            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                                ...
                                <div class="rtl:md:text-start">
                                    ...
                                    @endif
                                    <a class="mx-1 ltr:md:text-end" title="Add Package" href="{{ route('packages.add') }}"
                                        style="float: inline-end;">
                                        <button type="submit"
                                            class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                            <i class="ri-add-box-fill"></i> Add Package
                                        </button>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="overflow-x-auto">
                                @if (!$Packages->isEmpty())
                                    <form id="bulkDeleteForm" method="POST"
                                        action="{{ route('packages.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                        ...
                                        <table class="w-full whitespace-nowrap" id="customerTable">
                                            <thead class="bg-slate-100 dark:bg-zink-600">
                                                <tr>
                                                    ...
                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="mrp_price">MRP Price</th>

                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="Tests">Tests</th>

                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="fasting_required">Fasting Required</th>

                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="are_name">Logo</th>
                                                ...
                                                @foreach ($Packages as $plan)
                                                    <tr>
                                                        <th class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500"
                                                            ...
                                                            <input
                                                                class="border rounded-sm appearance-none cursor-pointer size-4 bg-slate-100 border-slate-200 dark:bg-zink-600 dark:border-zink-500 checked:bg-custom-500 checked:border-custom-500 dark:checked:bg-custom-500 dark:checked:border-custom-500 checked:disabled:bg-custom-400 checked:disabled:border-custom-400"
                                                                type="checkbox" name="package_ids[]"
                                                                value="{{ $plan->id }}">
                                                        </th>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 id">
                                                            {{ $i + $Packages->perPage() * ($Packages->currentPage() - 1) }}
                                                        </td>
                                                        ...
                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 name">
                                                            {{ $plan->name }}</td>
                                                        ...
                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 MRP_Price">
                                                            {{ $plan->MRP_Price }}</td>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 Tests">
                                                            {{ $plan->Tests }}</td>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 Tests">
                                                            {{ $plan->fasting_required == 1 ? 'Yes' : 'No' }}</td>
                                                        ...
                                                        <td
                                                            ...
                                                            @if ($plan->logo)
                                                                <img class="img-fluid" style="height:50px;width:50px;"
                                                                    src="{{ asset('upload/logo') . '/' . $plan->logo }}"
                                                                    alt="">
                                                            @else
                                                                <img class="img-fluid" style="height:50px;width:50px;"
                                                            ...
                                                            <div class="flex gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="Edit"
                                                                        href="{{ route('packages.edit', $plan->id) }}">
    ...
    <script>
        function checkAll(source) {
            checkboxes = document.querySelectorAll('input[name="package_ids[]"]');
            checkboxes.forEach(checkbox => checkbox.checked = source.checked);
        }

        function confirmBulkDelete() {
            const selected = document.querySelectorAll('input[name="package_ids[]"]:checked');
        ...
        function confirmSingleDelete(id) {
            Swal.fire({
                ...
                if (result.value) {
                    fetch('{{ route('packages.delete', ['id' => '__id__']) }}'.replace('__id__', id), {
                            method: 'DELETE',
                            headers: {
