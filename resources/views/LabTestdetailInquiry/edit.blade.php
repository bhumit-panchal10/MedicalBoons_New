...
@section('title', 'Edit Lab Test Detail Inquiry')
@section('content')
    {!! Toastr::message() !!}
                                ...
                                <div class="rtl:md:text-start">
                                    <div class="bg-white shadow rounded-md dark:bg-zink-600">
                                        <div
                                            ...
                                            <a
                                                href="{{ route('LabTestinquiryReport.detail', ['id' => $id, 'memberid' => $memberid]) }}">
                                                <button type="button"
                                        ...
                                        <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                                            <form onsubmit="return validateFile()" class="tablelist-form"
                                                action="{{ route('LabTestinquiryReport.detail_update', $data->LabReport_Request_detail_id) }}"
                                                method="POST">
                                                @csrf

                                                <div class="grid grid-cols-3 gap-4">
                                                    <input type="hidden" name="LabReport_Request_detail_id"
                                                        value="{{ $data->LabReport_Request_detail_id }}">
                                                    <input type="hidden" name="memberid" value="{{ $memberid }}">
                                                    <input type="hidden" name="labreqmasterid"
                                                        value="{{ $data->LabReport_Request_Master_id }}">

                                                    <div class=" mb-3">
                                                        <label for="email-field" class="">Lab Test Category<span
                                                                class="text-red-500"> *</span></label>
                                                        <select name="edit_lab_test_category_id"
                                                            id="edit_lab_test_category_id" required
                                                            class="form-select border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                                                            <option selected="" value="">Select Lab Test Category
                                                            </option>
                                                            @foreach ($LabTestCategory as $lab)
                                                                <option value="{{ $lab->Lab_Test_Category_id }}"
                                                                    {{ old('edit_lab_test_category_id', $data->Lab_test_category_id ?? '') == $lab->Lab_Test_Category_id ? 'selected' : '' }}>
                                                                    {{ $lab->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <div class=" mb-3">
                                                        <label for="email-field" class="">Lab Test Name<span
                                                                class="text-red-500"> *</span></label>
                                                        <select name="lab_test_id" id="edit_lab_test_id" required
                                                            class="form-select border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                                                            <option selected="" value="">Select Lab Test Name
                                                            </option>
                                                            @foreach ($labstest as $lab)
                                                                <option value="{{ $lab->Lab_Test_Master_id }}"
                                                                    {{ old('lab_test_id', $data->Lab_test_master_id ?? '') == $lab->Lab_Test_Master_id ? 'selected' : '' }}>
                                                                    {{ $lab->Test_Name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <div class="mb-3">
                                                        <label for="email-field" class="">Family Member Name<span
                                                                class="text-red-500">*</span></label>
                                                        <select name="family_member_id" id="edit_family_member_id" required
                                                            class="form-select border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                                                            <option selected="" value="">Select Family Member Name
                                                            </option>
                                                            @foreach ($familymembers as $familymeb)
                                                                <option value="{{ $familymeb->family_member_id }}"
                                                                    {{ old('family_member_id', $data->family_member_id ?? '') == $familymeb->family_member_id ? 'selected' : '' }}>
