...
@section('content')
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                ...
                <ul class="flex items-center gap-2 text-sm font-normal shrink-0">
                    ...
                    <li
                        class="relative before:content-['\ea54'] before:font-remix ltr:before:-right-1 rtl:before:-left-1  before:absolute before:text-[18px] before:-top-[3px] ltr:pr-4 rtl:pl-4 before:text-slate-400 dark:text-zink-200">
                        <a href="{{ route('B2B_User.index') }}" class="text-slate-400 dark:text-zink-200">Associate
                            List</a>
                    </li>
                                ...
                                <div class="rtl:md:text-start">
                                    <div class="bg-white shadow rounded-md dark:bg-zink-600">
                                        <div
                                            ...
                                            <h5 class="text-16" id="exampleModalLabel">Edit Associate User</h5>
                                        ...
                                        <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                                            <form onsubmit="return validateFile()" class="tablelist-form"
                                                action="{{ route('B2B_User.update') }}" method="POST">
                                                @csrf

                                                <div class="grid grid-cols-3 gap-4">
                                                    <input type="hidden" name="B2B_Users_id" value="{{ $data->Users_id }}">
    ...
    <script>
        function getsearchparentid(selectedParentId = null, B2BUserid = null) {
            var Main_parent_id = $("#Main_parent_id").val();
            var url = "{{ route('B2B_User.parent_id_mapping', ':Main_parent_id') }}";
        ...
        $(document).ready(function() {
            ...
            let B2BUserid = "{{ $data->Users_id ?? '' }}";

            let selectedParentId = "";
            @if (isset($data))
                @if ($data->Parent_id == 0)
                    selectedParentId = "{{ $data->Users_id ?? '' }}";
                @else
            ...
            if (selectedMainParentId) {
                ...
                getsearchparentid(selectedParentId, B2BUserid); // ✅ pass parent ID
            }
        });
    </script>
