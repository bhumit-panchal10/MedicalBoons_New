...
@section('content')
    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">LabMaster List</h5>
                </div>
                                ...
                                <div class="rtl:md:text-start">
                                    <div class="bg-white shadow rounded-md dark:bg-zink-600">
                                        <div
                                            ...
                                            <h5 class="text-16" id="exampleModalLabel">Add LabMaster</h5>
                                        </div>
                                        <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                                            <form method="POST" onsubmit="return validateFile()"
                                                action="{{ route('lab_master.store') }}" id="registerForm"
                                                enctype="multipart/form-data">
                                                @csrf
                                                <div class="mb-3">
                                                    <label for="email-field"
                                                        class="inline-block mb-2 text-base font-medium">Name<span
                                                            class="text-red-500"> *</span> </label>
                                                    <input type="text" id="email-field" name="labname" maxlength="50"
                                                ...
                                                <div class="mb-3">
                                                    <label for="email-field"
                                                        class="inline-block mb-2 text-base font-medium">Address<span
                                                            class="text-red-500"> *</span> </label>
                                                    <textarea id="email-field" name="address" maxlength="50"
                                                        class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                        placeholder="Enter Address" required autocomplete="off" autofocus></textarea>
                                                </div>
                            ...
                            <div class="overflow-x-auto">
                                @if (!$LabMasters->isEmpty())
                                    <form id="bulkDeleteForm" method="POST"
                                        action="{{ route('lab_master.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                        ...
                                        <table class="w-full whitespace-nowrap" id="customerTable">
                                            ...
                                            <tbody class="list form-check-all">
                                                <?php $i = 1; ?>
                                                @foreach ($LabMasters as $LabMaster)
                                                    <tr class="text-center">
                                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500"
                                                            scope="row">
                                                            <input
                                                                class="border rounded-sm appearance-none cursor-pointer size-4 bg-slate-100 border-slate-200 dark:bg-zink-600 dark:border-zink-500 checked:bg-custom-500 checked:border-custom-500 dark:checked:bg-custom-500 dark:checked:border-custom-500 checked:disabled:bg-custom-400 checked:disabled:border-custom-400"
                                                                type="checkbox" name="Lab_Master_ids[]"
                                                                value="{{ $LabMaster->Lab_Master_id }}">
                                                        </td>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 id">
                                                            {{ $i + $LabMasters->perPage() * ($LabMasters->currentPage() - 1) }}
                                                        </td>
                                                        ...
                                                        <td
                                                            ...
                                                            <div class="items-center">
                                                                <div
                                                                    ...
                                                                    @if ($LabMaster->iStatus == 1)
                                                                        <input type="checkbox" name="greenIconSwitch"
                                                                            id="greenIconSwitch{{ $LabMaster->Lab_Master_id }}"
                                                                            ...
                                                                            onchange="updateStatus(<?= $LabMaster->iStatus ?>,<?= $LabMaster->Lab_Master_id ?>);"
                                                                            checked="">
                                                                    @else
                                                                        <input type="checkbox" name="greenIconSwitch"
                                                                            id="greenIconSwitch{{ $LabMaster->Lab_Master_id }}"
                                                                    ...
                                                                    @endif
                                                                    <label
                                                                        for="greenIconSwitch{{ $LabMaster->Lab_Master_id }}"
                                                            ...
                                                            <div class="gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="Edit" href="#"
                                                                        ...
                                                                        onclick="getEditData(<?= $LabMaster->Lab_Master_id ?>)">
                                                                        <i class="ri-edit-2-fill"></i>
                                                                    </a>

                                                                    <a class="mx-1" title="Delete" href="#"
                                                                        onclick="confirmSingleDelete({{ $LabMaster->Lab_Master_id }})">
                ...
                <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
                    ...
                    <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                        <form class="tablelist-form" onsubmit="return EditvalidateFile()"
                            action="{{ route('lab_master.update') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" id="labmasterid" name="labmasterid" />
                            <div class="mb-3">
                                <label for="Countryname-field" class="inline-block mb-2 text-base font-medium">Service
                                    Name
                                    <span class="text-red-500"> *</span>
                                </label>
                                <input type="text" name="lab_name" id="Edit_lab_name" maxlength="50"
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    placeholder="Enter Lab Name" required autocomplete="off" autofocus>
                            </div>
                            ...
                            <div class="mb-3">
                                <label for="email-field" class="inline-block mb-2 text-base font-medium">Address<span
                                        class="text-red-500"> *</span> </label>
                                <textarea id="EditAddress" name="EditAddress" maxlength="50"
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    placeholder="Enter Address" required autocomplete="off" autofocus></textarea>
                            </div>
    ...
    <script>
        function getEditData(id) {
            var url = "{{ route('lab_master.edit', ':id') }}";
            ...
            if (id) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        console.log(obj);
                        $("#Edit_lab_name").val(obj.name);
                        $("#EditAddress").val(obj.address);
                        $('#labmasterid').val(obj.Lab_Master_id);
                    },
                    error: function(xhr) {
    ...
    <script>
        function checkAll(source) {
            checkboxes = document.querySelectorAll('input[name="Lab_Master_ids[]"]');
            checkboxes.forEach(checkbox => checkbox.checked = source.checked);
        }

        function confirmBulkDelete() {
            const selected = document.querySelectorAll('input[name="Lab_Master_ids[]"]:checked');
        ...
        function confirmSingleDelete(id) {
            Swal.fire({
                ...
                if (result.value) {
                    fetch('{{ route('lab_master.delete', ['id' => '__id__']) }}'.replace('__id__', id), {
                            method: 'DELETE',
                            headers: {
                        ...
                        .then(data => {
                            if (data.success) {
                                Swal.fire({
                                    ...
                                    window.location.href = '{{ route('lab_master.index') }}';
                                });
                                // location.reload());
                            } else {
                                Swal.fire({
    ...
    <script>
        function updateStatus(status, labmasterId) {
            //$("p").toggle();
            $('#loading').css("display", "block");
            $.ajax({
                type: 'POST',
                url: "{{ route('lab_master.updateStatus') }}",
                data: {
                    status: status,
