...
@section('content')
    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Sub Service List</h5>
                </div>
                                ...
                                <div class="rtl:md:text-start">
                                    <div class="bg-white shadow rounded-md dark:bg-zink-600">
                                        <div
                                            ...
                                            <h5 class="text-16" id="exampleModalLabel">Add Sub Service</h5>
                                        </div>
                                        <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                                            <form method="POST" onsubmit="return validateFile()"
                                                action="{{ route('sub_service.store') }}" id="registerForm"
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <select id="category-field" name="serviceid"
                                                ...
                                                <div class="mb-3">
                                                    <label for="email-field"
                                                        class="inline-block mb-2 text-base font-medium">Sub Service
                                                        Name<span class="text-red-500"> *</span> </label>

                                                    <input type="text" id="email-field" name="Subservicename"
                                                        maxlength="150"
                                                        class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                        placeholder="Enter Sub Service Name" required autocomplete="off"
                                                        autofocus>
                            ...
                            <div class="overflow-x-auto">
                                @if (!$subServices->isEmpty())
                                    <form id="bulkDeleteForm" method="POST"
                                        action="{{ route('sub_service.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                                ...
                                                @foreach ($subServices as $subService)
                                                    <tr class="text-center">
                                                        ...
                                                        <td
                                                            ...
                                                            {{ $subService->service->name ?? '-' }}</td>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ $subService->subservice_name }}</td>
                                                        ...
                                                        <td
                                                            ...
                                                            <div class="items-center">
                                                                <div
                                                                    ...
                                                                    @if ($subService->iStatus == 1)
                                                                        <input type="checkbox" name="greenIconSwitch"
                                                                            id="greenIconSwitch{{ $subService->sub_service_id }}"
                                                                            ...
                                                                            onchange="updateStatus(<?= $subService->iStatus ?>,<?= $subService->sub_service_id ?>);"
                                                                            checked="">
                                                                    @else
                                                                        <input type="checkbox" name="greenIconSwitch"
                                                                            id="greenIconSwitch{{ $subService->sub_service_id }}"
                                                                    ...
                                                                    @endif
                                                                    <label
                                                                        for="greenIconSwitch{{ $subService->sub_service_id }}"
                                                            ...
                                                            <div class="gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="Edit" href="#"
                                                                        ...
                                                                        onclick="getEditData(<?= $subService->sub_service_id ?>)">
                                                                        <i class="ri-edit-2-fill"></i>
                                                                    </a>

                                                                    <a class="mx-1" title="Delete" href="#"
                                                                        onclick="confirmSingleDelete({{ $subService->sub_service_id }})">
            ...
            <div id="EditModal" modal-center=""
                ...
                <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
                    <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
                        <h5 class="text-16" id="exampleModalLabel">Edit Sub Service</h5>
                    ...
                    <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                        <form class="tablelist-form" onsubmit="return EditvalidateFile()"
                            action="{{ route('sub_service.update') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <select id="editserviceid" name="editserviceid"
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    required>
                                    <option value="" disabled selected>Select a Category</option>
                                    @foreach ($Services as $Service)
                            ...
                            <div class="mb-3">
                                <label for="email-field" class="inline-block mb-2 text-base font-medium">Sub Service
                                    Name<span class="text-red-500"> *</span> </label>

                                <input type="text" id="editsubservicename" name="editsubservicename" maxlength="150"
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
    ...
    <script>
        function getEditData(id) {
            var url = "{{ route('sub_service.edit', ':id') }}";
            ...
            if (id) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        console.log(obj);
                        $("#Subserviceid").val(obj.sub_service_id);
                        $("#EditDescription").val(obj.sub_description);
                        $('#editserviceid').val(obj.service_id);
                        $('#editsubservicename').val(obj.subservice_name);
                    },
                    error: function(xhr) {
    ...
    <script>
        function updateStatus(status, sub_service_id) {
            //$("p").toggle();
            $('#loading').css("display", "block");
            $.ajax({
                type: 'POST',
                url: "{{ route('sub_service.updateStatus') }}",
                data: {
                    status: status,
    ...
    <script>
        function checkAll(source) {
            checkboxes = document.querySelectorAll('input[name="sub_service_Ids[]"]');
            checkboxes.forEach(checkbox => checkbox.checked = source.checked);
        }

        function confirmBulkDelete() {
            const selected = document.querySelectorAll('input[name="sub_service_Ids[]"]:checked');
        ...
        function confirmSingleDelete(id) {
            Swal.fire({
                ...
                if (result.value) {
                    fetch('{{ route('sub_service.delete', ['id' => '__id__']) }}'.replace('__id__', id), {
                            method: 'DELETE',
                            headers: {
                        ...
                        .then(data => {
                            if (data.success) {
                                Swal.fire({
                                    ...
                                    window.location.href = '{{ route('sub_service.index') }}';
                                });
                                // location.reload());
                            } else {
                                Swal.fire({
    ...
    <script>
        function validateFile() {
            ...
            var fileExtension = document.getElementById('SubCategoryicon').value.split('.').pop().toLowerCase();
    ...
    <script>
        function EditvalidateFile() {
            //alert('hello');
            var allowedExtension = ['jpeg', 'jpg', 'png', 'webp', ''];
            var fileExtension = document.getElementById('EditSubCategoryicon').value.split('.').pop().toLowerCase();
            var isValidFile = false;
            var image = document.getElementById('EditSubCategoryicon').value;
            for (var index in allowedExtension) {
                if (fileExtension === allowedExtension[index]) {
                    isValidFile = true;
                    break;
